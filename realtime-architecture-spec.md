...
**Two write-time guards added alongside the F-09 fix (§14 row 10):** `PUT /channels/{id}/filters` now caps a channel at 50 rules (`FilterEngine.MAX_RULES_PER_CHANNEL`) and compiles each rule's `field` as a JSONPath (`JsonPath.compile`) before accepting it, rejecting a syntactically invalid path with a 400 instead of letting it fail silently at evaluation time (a malformed field previously just never matched — extract() swallows it — which is honest per the "missing field is not an error" rule above, but unhelpful when the *path itself* is broken rather than merely absent from a given payload). The 50-rule figure is a reasonable default, not a load-tested one; see §14 row 10.
...
api_keys       (id, channel_id, key_hash, key_suffix, created_at, revoked_at)
...
- `key_suffix` on `api_keys` (M6b) — stores the last 4 characters of the raw API key for masked display (`rtk_...xxxx`) in the dashboard without revealing the full secret.
...
| 3 | Cold archive precision (§7) | `events_archive.received_at` (Instant, millisecond precision) can't fully express a Redis stream ID's sequence-number-level exclusivity. Two events for the same channel landing in the same millisecond aren't perfectly disambiguated by a timestamp column alone. | M4 | Partially resolved in M6c (F-07: `GreaterThanEqual` + downstream sequence filter eliminates same-ms lower bound event loss; full sortable ID column deferred) |
| 4 | Filter rule storage (§5) | Channel filter rules still live in the in-memory `ChannelFilterStore`, not persisted alongside the now-real `channels` table. A deliberate scope boundary — M5's and M6b's ships list excluded filter rule CRUD. | M3 (created), reassessed at M5/M6b | Open — planned for M8 / M8c (F-10: persisted rules + Redis pub/sub invalidation cache) |
...
| 10 | Filter rule limits (§5) | `MAX_RULES_PER_CHANNEL = 50` and per-rule JSONPath compile validation were added as proactive hardening alongside the F-09 fix — not derived from load-test data, and not requested by a named review finding. The cap protects against unbounded per-event `JsonPath` evaluation cost (Netty event-loop CPU), but 50 is a reasonable-sounding default, not a measured one. | Between M6b and M6c | Open — revisit the exact figure once M10 load-test data exists, ideally alongside F-17's parse-once refactor at M7a, since both bear on the same per-event filter-evaluation cost |
| 11 | Stream MAXLEN cap (§3) | Ingest `XADD` previously omitted a `MAXLEN` limit, allowing a single high-volume channel to grow unbounded in Redis memory before the scheduled 24h retention sweep. | M1, surfaced by peer review | Resolved in M6c (F-01: approximate `MAXLEN ~` cap added to `XADD`, default ~50k via `app.stream.maxlen`) |
| 12 | Lettuce connection head-of-line blocking (§8) | Hypothesized that `XREAD BLOCK 15s` over the default shared Lettuce connection would stall subsequent ingest `XADD`s behind blocked WS readers. Empirically tested in M6c: Lettuce/Spring Data Redis automatically provisions a dedicated connection (`flags=b`) per blocking stream read; the shared connection (`flags=N`) processes `XADD`s without delay (ingest p50/p99 remained flat across 0 vs 3 blocked WS sessions). | Pre-M7 review | Resolved in M6c (Hypothesis disproven by empirical measurement; no dedicated connection factory needed) |
| 13 | WS resume Postgres fallback (§3) | §3 sequence diagram depicts WS gateway falling through to Postgres cold archive when `last_id` predates the hot stream window; `ChannelWebSocketHandler` currently queries Redis only, silently dropping pre-stream history on resume. | M2, surfaced by peer review | Open — planned for M8 / M8c (F-06: shared resume-position resolver with `ReplayService`) |
| 14 | Slow-consumer buffering & gap frame (§9) | Spec promises a bounded per-connection outbound buffer (~1,000 events) with drop-oldest and an explicit `{"type":"gap","dropped":N}` control frame on overflow. Currently sends directly to Netty channel with unbounded buffering. | Spec §9, surfaced by peer review | Open — planned for M7 / M7b (F-18: slow-consumer handling milestone) |
